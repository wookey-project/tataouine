...
def PrintUsage():
    ...
    print(u'\nkey2c\n\n\tUsage:\t{} certificate.der keys.der, OR key2c hmac_to_save_file\n'.format(executable))
    sys.exit(-1)
...
def Key2C(argv):
    ...
    #### Handle HMAC
        ...
        # We are provided the file with HMAC of the plaform keys on the command
        # line. Read it and save it
        if not os.path.isfile(argv[1]):
            print(u'\nFile "{}" does not exist'.format(argv[1]))
            PrintUsage()
        fido_hmac  = read_in_file(argv[1])
        text = "static const unsigned char fido_hmac["+str(len(fido_hmac))+"] = { \n\t"
        line = 1
        for byte in fido_hmac:
            text += "0x%02x, " % ord(byte)
            if line % 8 == 0:
                text += "\n\t"
            line += 1
