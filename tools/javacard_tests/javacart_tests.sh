...
/bin/bash -c "cd /build && rm -rf /build/build && rm -rf /build/private/ && source setenv.sh && make wookey-v2/graphic_hs_defconfig && make genkeys && make javacard_compile"
...
/bin/bash -c "source setenv.sh && make sign tosign=flop chunksize=4096 version=1.0.0.1 && make sign tosign=flip chunksize=4096 version=1.0.0.2"
/bin/bash -c "source setenv.sh && make verify toverify=flip:flop"
