...
quiet_cmd_kconf_drv_gen = KCONF
      cmd_kconf_drv_gen = for i in $(shell find drivers/socs/*/ -mindepth 1 -maxdepth 1 -type d -exec basename {} \;); do upper=`/bin/echo "$$i" |tr '[:lower:]' '[:upper:]'`; /bin/echo "config USR_DRV_$${upper}_CFLAGS" >>$(PROJ_FILES)/drivers/Kconfig.gen; /bin/echo "  string" >>$(PROJ_FILES)/drivers/Kconfig.gen; /bin/echo "  default \" -I../../drivers/socs/$(CONFIG_SOCNAME)/$$i/api \" if USR_DRV_$$upper" >>$(PROJ_FILES)/drivers/Kconfig.gen; /bin/echo "" >>$(PROJ_FILES)/drivers/Kconfig.gen; done

quiet_cmd_kconf_lib_gen = KCONF
      cmd_kconf_lib_gen = for i in $(shell find libs/ -mindepth 1 -maxdepth 1 -type d -exec basename {} \;); do upper=`/bin/echo "$$i" |tr '[:lower:]' '[:upper:]'`; /bin/echo "config USR_LIB_$${upper}_CFLAGS" >>$(PROJ_FILES)/libs/Kconfig.gen; /bin/echo "  string" >>$(PROJ_FILES)/libs/Kconfig.gen; /bin/echo "  default \" -I../../libs/$$i/api \" if USR_LIB_$$upper" >>$(PROJ_FILES)/libs/Kconfig.gen; /bin/echo "" >>$(PROJ_FILES)/libs/Kconfig.gen; done
